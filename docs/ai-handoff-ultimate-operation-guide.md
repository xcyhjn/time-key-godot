...
| `scene/in_scene/timecoin_ui.gd` | 约 469 行，已拆出 `TimecoinGlobalBridge.gd` 和 `TimecoinHourglassShaderController.gd`，分别处理 `GlobalTimecoin` 查找和沙漏 shader 材质/震动参数写入。旧 `_get_timecoin_singleton()`、`_initialize_shader_material()`、`start_hourglass_shake()`、`stop_hourglass_shake()` 等入口仍保留。 | 后续如继续，优先审查获得/消耗/不足动画 runner；不要同批改时间币数值来源、信号协议和多个动画收尾状态。 |
