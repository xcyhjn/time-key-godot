...
当前模块化工作已经完成多条主线。当前已拆模块共 `124` 个，`docs/modularized-files-ultimate-operation-guide.md` 对这些模块的覆盖缺失数为 `0`。
...
| `scene/in_scene/rewards/*.gd` | 奖励页已拆出 45 个模块。Acquire/Remove 已接近页面编排；Shop 生成前依赖检查已收口，Craft 结果预览挂载、预览卡 UI 配置、预览 DraftCard 工厂复用和合成结果牌组写入处理已完成。 | Craft 预览创建链和确认写入链都不建议继续硬拆；下一步优先评估静态配置资源化，或转向拖拽放置完成流程。 |
...
| 优先级 | 文件 | 当前问题 | 首批建议 |
| --- | --- | --- | --- |
| 1 | `scene/in_scene/rewards/CraftReward.gd` | `_refresh_result_preview()` 已拆出结果预览挂载，`_create_preview_card()` 已复用 DraftCard 工厂并拆出预览卡 UI 配置；`_apply_crafting_result_to_deck()` 已拆出移除索引计算和结果写入处理。剩余内容主要是页面确认后的同步和关闭编排。 | 停止继续硬拆 Craft 页面流程；如果继续处理 Craft，优先评估配方表资源化，不要再拆确认关闭链。 |
...
```text
1. CraftReward.gd 的 CRAFTING_RECIPES 配方表资源化评估
...
Resource 适合承载“可复用、可 Inspector 调参、默认只读”的数据，不适合承载会在一局内频繁变化的运行态状态。当前最适合评估的候选是：

```text
P1：CraftReward.gd::CRAFTING_RECIPES -> CraftRecipeBook / CraftRecipeEntry
...
下一批如果选择 Resource 化，建议先做 `CraftReward.gd::CRAFTING_RECIPES`。它是静态配方表，已经通过 `CraftRecipeResolver.gd` 读取，边界比地图、商店和时间轴表现配置更清晰。
...
当前已拆模块共 124 个，docs/modularized-files-ultimate-operation-guide.md 覆盖缺失为 0。不要继续机械拆 hex_map.gd、in_scene.gd、AcquireReward.gd、RemoveReward.gd 或 CraftReward.gd 的确认关闭链。下一阶段优先处理：
1. scene/in_scene/rewards/CraftReward.gd 的 CRAFTING_RECIPES 配方表资源化评估
