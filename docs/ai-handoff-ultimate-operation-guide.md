...
| 6 | `scene/in_scene/tile.gd` | 地貌规则、状态组件、结算奖励、敌人意图、血量和贴图写入仍在实体脚本里；timeline shape 矩阵解析已拆到 `TileTimelineShapeParser.gd`，默认敌人意图 `TimelineAction` 构造已拆到 `TileIntentActionFactory.gd`，`altar.gd`、`iron_mine.gd`、`Animal_husbandry.gd` 与 `center_altar.gd` 的子类意图 `action_data` 字典构造已拆到 `TileIntentActionDataBuilder.gd`，血量 clamp、damage_rate 和主状态标签计算已拆到 `TileHealthStateRules.gd`，Broken 后死亡收尾执行已拆到 `TileDeathExecutionController.gd`，protected 受击吸收规则已拆到 `TileDamageProtectionRules.gd`，贴图数组归属切换判断已拆到 `TileTextureStateSelector.gd`。 | 下一步若继续 Tile，只能小批评估 `village.gd` 或 `radar.gd` 的 action_data 字典迁移，或重新审查子类 picker 策略；不要同批碰死亡收尾、贴图切换和 TimelineAction 数据契约。 |
...
2. scene/in_scene/tile.gd 的 `village.gd` 或 `radar.gd` action_data 字典小批迁移重新审查，或停止 Tile 转向其他大文件
3. scene/card/custom_card.gd 剩余函数的重新审查，避免硬拆出牌和选中状态链
4. scene/in_scene/timecoin_ui.gd 的获得/消耗/不足动画 runner
