...
当前模块化工作已经完成多条主线。当前已拆脚本模块共 `155` 个，另有 `4` 个默认 Resource 文件；`docs/modularized-files-ultimate-operation-guide.md` 对这些脚本和资源路径的覆盖缺失数为 `0`。
...
| `scene/card/custom_card.gd` | 约 597 行，`custom_card_modules/` 下已有 5 个模块：时间轴形状解析、六边形效果范围解析、选中态跟随视觉 presenter、hover shader presenter 和 tooltip bridge。旧 `_normalize_and_parse_shape()`、`_parse_hex_effect_range()`、`_process(delta)`、`_set_shader(active)` 与 `_request_tooltip()` 仍保留入口。 | 继续前先重新审查剩余函数；不要为了降行数硬拆 `_enter_state()`、选中/取消选中或 `play_card()`。 |
...
| 7 | `scene/card/custom_card.gd` | 卡牌数据解析、选中/取消选中、状态进入和出牌逻辑仍耦合；时间轴形状解析、六边形效果范围解析、选中跟随视觉、hover shader 参数写入和 tooltip 查找已拆到 `custom_card_modules/`。 | 继续前先重新审查剩余函数；避免直接改出牌逻辑。 |
...
当前已拆脚本模块共 155 个，另有 4 个默认 Resource 文件，docs/modularized-files-ultimate-operation-guide.md 覆盖缺失为 0。不要继续机械拆 hex_map.gd、in_scene.gd、AcquireReward.gd、RemoveReward.gd 或 CraftReward.gd 的确认关闭链。下一阶段优先处理：
