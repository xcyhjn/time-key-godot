...
当前模块化工作已经完成多条主线。当前已拆模块共 `122` 个，`docs/modularized-files-ultimate-operation-guide.md` 对这些模块的覆盖缺失数为 `0`。
...
| `scene/in_scene/rewards/*.gd` | 奖励页已拆出 43 个模块。Acquire/Remove 已接近页面编排；Shop 生成前依赖检查已收口，Craft 结果预览挂载已拆出。 | Shop 单个商品生成编排已判断牵动状态过多，不建议硬拆；下一步继续评估 CraftReward 的异步预览创建边界，或转向拖拽放置完成流程。 |
...
| 优先级 | 文件 | 当前问题 | 首批建议 |
| --- | --- | --- | --- |
| 1 | `scene/in_scene/rewards/CraftReward.gd` | `_refresh_result_preview()` 已拆出结果预览挂载，但仍涉及配方判断和异步预览卡创建；`_apply_crafting_result_to_deck()` 剩余真实牌组写入和同步。 | 继续只评估 `_create_preview_card()` 或异步预览创建是否有单一边界；暂不同时改牌组写入。 |
...
```text
1. CraftReward.gd::_create_preview_card() 或异步预览创建边界
...
当前已拆模块共 122 个，docs/modularized-files-ultimate-operation-guide.md 覆盖缺失为 0。不要继续机械拆 hex_map.gd、in_scene.gd、AcquireReward.gd 或 RemoveReward.gd。下一阶段优先处理：
1. scene/in_scene/rewards/CraftReward.gd 的 _create_preview_card() 或异步预览创建边界评估
...
优先从 CraftReward.gd::_create_preview_card() 或异步预览创建边界开始评估。CraftReward.gd::_refresh_result_preview() 的结果预览挂载已经拆到 CraftResultPreviewPresenter.gd。ShopManager.gd::_generate_shop_items() 的单个商品生成编排已经判断会传入 draft_card_factory、temp_pile、deck_manager、价格、UI 注册和异步数据提取等过多状态，不建议硬拆；若继续看 Shop，只评估临时牌堆生命周期等更小边界。不要重复拆已经完成的节点桥接、tooltip、CardManager 查找、临时牌堆、DraftCard 数据写入、生成依赖检查和只读牌组来源模块。
```
