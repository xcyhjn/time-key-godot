...
当前模块化工作已经完成多条主线。当前已拆脚本模块共 `178` 个，另有 `4` 个默认 Resource 文件；`docs/modularized-files-ultimate-operation-guide.md` 对这些脚本和资源路径的覆盖缺失数为 `0`。
...
| `scene/in_scene/timeline/timeline_ui.gd` | 约 800 行，`timeline/ui_modules/` 下已有 16 个模块，`timeline/resources/` 下已有 `TimelineVisualConfig` 与默认资源。已拆布局、背景网格、预览样式、TimelineManager 查找、视觉配置读取、敌方意图 overlay、放置动画、行动容器几何、行动方块视觉节点、整体形状视觉层、清理动画残影创建、残影 tween 播放、行动块 hover 状态通知和入场动画编排状态，并已完成首批纯视觉参数 Resource 化。 | 行动块生成仍耦合创建时机、方格/容器创建和信号连接；不要硬拆 `_on_action_placed()`。后续如继续时间轴，只做小的纯表现配置补充或暂停转向其他文件。 |
...
| `scene/in_scene/enermy/enemy_intent_presentation_controller.gd` | 约 542 行，已拆出 `EnemyIntentTooltipTextBuilder.gd`、`EnemyIntentStatusKeywordTooltipPresenter.gd` 和 `EnemyIntentTooltipPositionHelper.gd`，分别处理主 tooltip 文本行、状态关键词副 tooltip 以及主 tooltip 屏幕位置计算。 | Tooltip 低风险表现面已基本收口。后续若继续，只谨慎评估引用查找 bridge；不要重复拆文本 builder、状态关键词副 tooltip presenter 或主 tooltip 定位 helper，也不要同批动 EnemyIntentResolver、TimelineManager 或地图/时间轴联动规则。 |
...
| 优先级 | 文件 | 当前问题 | 首批建议 |
| --- | --- | --- | --- |
| 1 | `scene/out_scene/out_scene_map_exp.gd` | 局外地图初始化、移动和切场景 executor 仍在一个主控里；房间结算 payload、章节揭示地块动画、切场前 payload 注入和镜头限制写入已拆出。 | 房间完成状态回写需要先定数据契约；短期不要同批改地图移动和切场景 executor，也不要重复拆镜头限制。 |
| 2 | `scene/in_scene/timeline/timeline_ui.gd` | 布局、网格、预览、视觉配置读取、overlay、放置动画、行动容器几何、行动方块视觉节点、整体形状视觉层、清理动画残影创建、残影 tween 播放、行动块 hover 状态通知、入场动画编排状态和首批 `TimelineVisualConfig` 纯视觉参数资源化已完成。剩余行动块生成仍有创建时机、方格/容器创建和信号连接耦合。 | 行动块生成暂不硬拆；后续只在需要新增纯视觉调参时小批进入，不动 TimelineManager 数据。 |
...
| 6 | `scene/in_scene/tile.gd` | 地貌规则、状态组件、结算奖励、敌人意图、血量和贴图写入仍在实体脚本里；timeline shape 矩阵解析已拆到 `TileTimelineShapeParser.gd`，默认敌人意图 `TimelineAction` 构造已拆到 `TileIntentActionFactory.gd`，`altar.gd`、`iron_mine.gd`、`Animal_husbandry.gd`、`center_altar.gd`、`village.gd` 与 `radar.gd` 的子类意图 `action_data` 字典构造已拆到 `TileIntentActionDataBuilder.gd`，血量 clamp、damage_rate 和主状态标签计算已拆到 `TileHealthStateRules.gd`，Broken 后死亡收尾执行已拆到 `TileDeathExecutionController.gd`，protected 受击吸收规则已拆到 `TileDamageProtectionRules.gd`，贴图数组归属切换判断已拆到 `TileTextureStateSelector.gd`。 | Tile 标准 action_data 方向已收口；后续若继续 Tile，只单独重新审查子类 picker 策略，不要同批碰死亡收尾、贴图切换和 TimelineAction 数据契约。 |
...
| 8 | `scene/in_scene/enermy/enemy_intent_presentation_controller.gd` | 主 tooltip 文本 builder、状态关键词副 tooltip presenter 和主 tooltip 定位 helper 已拆出；引用查找、hover phase 判断、主 tooltip 写入、地图和时间轴表现仍耦合。 | Tooltip 表现方向先暂停；后续若继续，只谨慎评估引用查找 bridge，不动地图/时间轴联动规则。 |
...
| 10 | `scene/in_scene/timecoin_ui.gd` | 全局 Timecoin 查找已拆到 `TimecoinGlobalBridge.gd`，沙漏 shader 控制已拆到 `TimecoinHourglassShaderController.gd`，普通动画抖动片段已拆到 `TimecoinShakeTweenBuilder.gd`，获得/消耗/不足反馈动画片段已拆到 `TimecoinFeedbackAnimationRunner.gd`；数值显示、动画触发入口和 tween 清理仍在 UI 脚本里。 | 后续如继续，只评估 `active_tweens` 清理 controller；也可转向 `enemy_intent_presentation_controller.gd` 的引用查找 bridge。 |
...
已拆低风险边界包括展开遮罩表现、背景网格构建、网格交互表现、顶部锚点布局、网格预览样式、TimelineManager 查找、敌方意图 overlay、行动方格放置动画、行动容器几何计算、行动方块视觉节点创建、行动整体形状视觉层、清理动画残影创建、残影 tween 播放、行动块 hover 状态通知和入场动画编排状态。
...
`TimecoinHourglassShaderController.gd` 只为 `timecoin_ui.gd` 准备沙漏图标的 ShaderMaterial 并写入震动参数。旧 `_initialize_shader_material()`、`start_hourglass_shake()`、`stop_hourglass_shake()`、`set_shake_intensity()` 和 `set_shake_frequency()` 入口仍保留并转发给 controller；它不连接时间币信号，也不处理获得、消耗或不足动画。
...
`EnemyIntentStatusKeywordTooltipPresenter.gd` 只为 `enemy_intent_presentation_controller.gd` 创建、定位和销毁状态关键词副 tooltip。旧 `_rebuild_status_keyword_tooltips()`、`_create_status_keyword_panel()`、`_position_status_keyword_tooltips()` 与 `_hide_status_keyword_tooltips()` 仍保留并转发；presenter 不读取敌人状态，不写入主 tooltip 文本，不判断 hover phase，也不驱动地图或时间轴表现。

`EnemyIntentTooltipPositionHelper.gd` 只为 `enemy_intent_presentation_controller.gd` 计算主 tooltip 的屏幕位置和边界 clamp。旧 `_position_intent_tooltip()` 仍负责 source stack 查找、主 tooltip panel size 选择、`MainBoard.set_cursor_tooltip_position()` 调用和状态关键词副 tooltip 重定位；helper 不读取节点树，不写入 `MainBoard`，不创建或销毁 tooltip。

`TimelineVisualConfigReader.gd` 只为 `timeline_ui.gd` 从视觉配置 Resource 读取静态表现参数并提供 fallback。旧 `_get_visual_config_value()`、`_get_visual_color()`、`_get_visual_vector2()`、`_get_visual_float()`、`_get_visual_int()`、`_get_visual_bool()` 和 `_get_enemy_intent_timeline_shader()` 仍保留入口并转发给 reader；reader 不创建行动块、不读写 `TimelineManager`，也不播放动画。

`TimelineIntroPlaybackController.gd` 只为 `timeline_ui.gd` 维护时间轴入场动画的播放状态，并转发 `TimelineIntroAnimator` 的 setup、背景格子入场、行动入场和停止请求。旧 `_setup_timeline_intro_animator()`、`play_intro()`、`is_intro_in_progress()`、`_should_play_action_intro()` 与 `_on_timeline_cleared()` 中的停止入口仍保留；controller 不创建行动容器，不播放具体 Tween，不修改 `TimelineManager`，也不决定敌人意图规则。

`TimelineEnemyIntentCandidateCollector.gd` 只为 `TimelineManager.gd` 从敌人列表收集本回合可进入时间轴的意图候选。旧 `_collect_enemy_intent_candidates()` 仍保留并转发；collector 只做协议检查、意图开关、`can_generate_intent(hex_map)` 和 shape 缓存，不排序候选，不寻找放置位置，不创建 `TimelineAction`，不修改 `grid`，也不决定目标地块。

`TimelineEnemyIntentPrioritySelector.gd` 只为 `TimelineManager.gd` 处理敌方意图候选优先级读取、降序优先级列表、同级候选过滤和同级可放置候选选择。旧 `_get_enemy_intent_priority()`、`_get_sorted_priority_values()`、`_filter_candidates_by_priority()` 和 `_pick_placeable_candidate()` 仍保留并转发；selector 不读取或修改 `grid`，不创建 `TimelineAction`，不调用 `place_action()`，也不决定目标地块。

`TimelineEnemyIntentTargetResolver.gd` 只为 `TimelineManager.gd` 把敌方意图声明的目标中心坐标映射成 `HexMap.stack_nodes` 里的地块节点。旧 `_resolve_intent_target_tile()` 仍保留并转发；resolver 不选择目标，不创建 `TimelineAction`，不修改 `HexMap.stack_nodes`，也不处理地图或时间轴表现。
...
`CustomCardTooltipBridge.gd` 只为 CustomCard 查找 `MainBoard`，并转发 `show_tooltip(card)` / `hide_tooltip(card)`。`custom_card.gd::_request_tooltip(should_show)` 仍保留旧入口，hover、holding、数值变化和视觉重置里的触发时机不变。

`CustomCardMapConditionalEffectBridge.gd` 只从 `MainBoard` 查找 `HexMap`，并转发 `update_all_stack_conditional_effects()`。`custom_card.gd::_update_map_conditional_effects()` 仍保留旧入口，选中和取消选中里的调用时机不变；它不计算条件效果，也不写地图数据。

下一批如果继续 `custom_card.gd`，先重新审查剩余函数，不要直接拆：

```text
_enter_state()
toggle_selection() / force_deselect()
play_card()
```
...
移动动画、切场景仍留在主文件。下一批如果继续局外地图，优先重新审查剩余函数；房间完成状态如果需要同时改 `tile_data`、`tile_features`、`view.tiles`、存档和切场返回，就暂停拆分并先定数据契约。
...
接管 `apply_tier_camera_limit()` 与 `_apply_sector_camera_limits()` 中的 `Camera2D.limit_*` 写入。镜头动画、镜头锁定、玩家移动、路径坍塌、进房、保存和切场景仍留在主文件。
...
当前已拆脚本模块共 178 个，另有 4 个默认 Resource 文件，docs/modularized-files-ultimate-operation-guide.md 覆盖缺失为 0。不要继续机械拆 hex_map.gd、in_scene.gd、AcquireReward.gd、RemoveReward.gd 或 CraftReward.gd 的确认关闭链。下一阶段优先处理：
1. scene/out_scene/out_scene_map_exp.gd 的房间完成状态实现前置设计：契约已评估，后续如实现必须新增独立状态字段，不复用 path_gone
2. enemy_intent_presentation_controller.gd 若继续只谨慎评估引用查找 bridge；如果继续 scene/in_scene/timecoin_ui.gd，只评估 active_tweens 清理 controller，不重复拆获得/消耗/不足动画 runner
3. scene/card/custom_card.gd 剩余函数的重新审查，避免硬拆出牌和选中状态链
4. scene/out_scene/out_scene_map_exp.gd 若继续，先重新审查剩余函数；不要重复拆镜头限制，不要同批碰地图移动和切场景 executor
...
out_scene_map_exp.gd 的房间结算 payload 消费已拆到 RoomResolutionController.gd，章节揭示地块动画已拆到 ChapterRevealAnimationRunner.gd，切场前 payload 注入已拆到 OutScenePayloadBridge.gd，镜头限制写入已拆到 OutSceneCameraLimitController.gd；房间完成状态回写契约已评估，当前没有既有字段可直接复用，path_gone 是路径坍塌记录，active_room_context 和 pending_room_resolution 是临时桥接，tile_data 仍是房间类型映射；后续如实现必须新增独立完成状态字段和 Saver 持久化，不要同批改地图移动和场景切换 executor，也不要重复拆镜头限制。timeline_ui.gd 已经拆出展开遮罩、背景网格、格子交互、顶部布局、网格预览、TimelineManager 查找、TimelineVisualConfigReader 视觉配置读取、敌方意图 overlay、行动方格放置动画、行动容器几何、行动方块视觉节点、整体形状视觉层、清理动画残影创建、残影 tween 播放、行动块 hover 状态通知和入场动画编排状态，并已完成 TimelineVisualConfig 首批纯视觉参数资源化；行动块生成暂不硬拆，后续不要重复拆视觉配置读取或 intro 状态转发。TimelineManager.gd 的敌方意图候选收集已拆到 TimelineEnemyIntentCandidateCollector.gd，敌方意图优先级读取、优先级降序列表、同级候选过滤和同级可放置选择已拆到 TimelineEnemyIntentPrioritySelector.gd，敌方意图目标中心坐标到 `HexMap.stack_nodes` 地块节点的映射已拆到 TimelineEnemyIntentTargetResolver.gd；旧 `_collect_enemy_intent_candidates()`、`_get_enemy_intent_priority()`、`_get_sorted_priority_values()`、`_filter_candidates_by_priority()`、`_pick_placeable_candidate()` 和 `_resolve_intent_target_tile()` 仍保留并转发，后续不要重复拆这三个模块，不要为了降行数硬拆 `generate_enemy_intents()`，也不要同批改 grid、place_action、TimelineAction.action_data 和 UI 表现。custom_card.gd 的节点查找已拆到 CustomCardNodeBridge.gd，描述解析已拆到 CustomCardDescriptionParser.gd，时间轴形状解析已拆到 CustomCardTimelineShapeParser.gd，六边形效果范围解析已拆到 CustomCardEffectRangeParser.gd，选中状态下的倾斜和阴影跟随已拆到 CustomCardSelectedVisualPresenter.gd，hover shader 参数写入已拆到 CustomCardHoverShaderPresenter.gd，tooltip MainBoard 查找与显隐转发已拆到 CustomCardTooltipBridge.gd，地图条件效果刷新已拆到 CustomCardMapConditionalEffectBridge.gd，旧入口仍写回 active_keywords、timeline_shape_coords/size/key 和 effect_range_offsets，`_process(delta)` 仍先判断 is_selected 再转发视觉参数，`get_parsed_description()`、`_set_shader(active)`、`_request_tooltip(should_show)`、`_update_map_conditional_effects()` 与节点查找旧入口仍保留，`_get_player_hand_container()` 也只把当前 `card_container` 与兜底玩家手牌解析交给 NodeBridge；后续不要重复拆 description parser/node bridge/shape/effect_range/selected follow/hover shader/tooltip bridge/map conditional bridge，继续前先重新审查剩余函数，不要硬拆 _enter_state、toggle_selection、force_deselect、apply_stat_modifier、return_to_hand 或 play_card，也不要同批统一 EffectProcessor 或敌人意图解析。timecoin_ui.gd 的全局时间币查找已拆到 TimecoinGlobalBridge.gd，沙漏 shader 控制已拆到 TimecoinHourglassShaderController.gd，普通动画抖动片段已拆到 TimecoinShakeTweenBuilder.gd，获得/消耗/不足反馈动画片段已拆到 TimecoinFeedbackAnimationRunner.gd；旧 `_get_timecoin_singleton()`、`_initialize_shader_material()`、`start_hourglass_shake()`、`stop_hourglass_shake()`、`_apply_shake_effect()`、`_play_gain_animation()`、`_play_consume_animation()` 和 `_play_warning_animation()` 等入口仍保留，后续不要重复拆 GlobalTimecoin 查找、沙漏 shader 控制、普通抖动片段或反馈动画 runner；如果继续 TimecoinUI，只单独评估 `active_tweens` 清理 controller，并保持数值来源和信号协议不变。enemy_intent_presentation_controller.gd 的主 tooltip 文本行组装已拆到 EnemyIntentTooltipTextBuilder.gd，状态关键词副 tooltip 创建、定位和销毁已拆到 EnemyIntentStatusKeywordTooltipPresenter.gd，主 tooltip 屏幕位置和边界 clamp 已拆到 EnemyIntentTooltipPositionHelper.gd；旧 `_show_intent_tooltip()` 仍负责写入 `MainBoard.cursor_tooltip`、显示 tooltip、重建状态关键词副 tooltip 和延迟定位，旧 `_position_intent_tooltip()` 仍负责 source stack 查找、主 tooltip panel size 选择、`MainBoard.set_cursor_tooltip_position()` 调用和状态关键词副 tooltip 重定位；旧 `_rebuild_status_keyword_tooltips()`、`_create_status_keyword_panel()`、`_position_status_keyword_tooltips()` 与 `_hide_status_keyword_tooltips()` 仍保留并转发。后续不要重复拆主 tooltip 文本 builder、状态关键词副 tooltip presenter 或主 tooltip 定位 helper；若继续，只谨慎评估引用查找 bridge，不要同批动 EnemyIntentResolver、EnemyIntentData、TimelineManager 或地图/时间轴联动规则。tile.gd 的 timeline shape 矩阵解析已拆到 TileTimelineShapeParser.gd，默认敌人意图 TimelineAction 构造已拆到 TileIntentActionFactory.gd，`altar.gd`、`iron_mine.gd`、`Animal_husbandry.gd`、`center_altar.gd`、`village.gd` 与 `radar.gd` 的子类意图 action_data 字典构造已拆到 TileIntentActionDataBuilder.gd，血量 clamp、damage_rate 和主状态标签计算已拆到 TileHealthStateRules.gd，Broken 后死亡收尾执行已拆到 TileDeathExecutionController.gd，protected 受击吸收规则已拆到 TileDamageProtectionRules.gd，贴图数组归属切换判断已拆到 TileTextureStateSelector.gd；旧 `_parse_matrix_shape()`、`parse_timeline_shape()`、`get_intent_shape()`、`get_timeline_shape_size()`、`set_timeline_shape()`、默认 `get_intent_action()`、`take_damage()`、`set_health()`、`State_Update()`、`die()` 和 `tex_toggle()` 仍保留入口；后续继续 Tile 时停止 action_data 方向，只单独重新审查子类 picker 策略，不要重复拆死亡收尾、protected 受击、贴图归属判断或同批改 TimelineAction 数据契约。DragShapeController.gd 的玩家 TimelineAction 创建已拆到 DragPlayerActionFactory.gd，时间轴提交已拆到 DragTimelineActionSubmitter.gd，成功卡牌视觉复原已拆到 DragSuccessCardVisualRestorer.gd，弃牌移动已拆到 DragSuccessDiscardMover.gd；成功后时间轴收起已确认只是现有 DragTimelineUiStateController 的单行调用，fallback 收尾牵动手牌、tooltip、时间轴和主状态，暂不继续拆。CraftReward.gd::_refresh_result_preview() 的结果预览挂载已经拆到 CraftResultPreviewPresenter.gd，预览卡尺寸/位置/tooltip 鼠标过滤已经拆到 CraftPreviewCardConfigurator.gd，_create_preview_card() 已复用 RewardDraftCardFactory 且剩余异步链不建议继续硬拆，_apply_crafting_result_to_deck() 已拆出索引计算和结果写入处理，CRAFTING_RECIPES 已资源化，剩余同步/关闭属于页面编排。ShopManager.gd 的商店定价已资源化为 ShopPricingConfig，时代权重已资源化为 ShopEraWeightConfig，隐藏临时牌堆创建已收口到 RewardTempPileFactory；_generate_shop_items() 的单个商品生成编排已经判断会传入 draft_card_factory、temp_pile、deck_manager、UI 注册和异步数据提取等过多状态，不建议硬拆。不要重复拆已经完成的节点桥接、tooltip、CardManager 查找、临时牌堆、DraftCard 数据写入、生成依赖检查、只读牌组来源、Craft 预览工厂复用、Craft 结果写入模块、Craft 配方资源、Shop 定价资源、Shop 时代权重资源、Shop 临时牌堆隐藏创建、Drag 玩家行动创建、Drag 时间轴提交、Drag 成功视觉复原、Drag 弃牌移动、TimelineUI 清理动画残影创建、TimelineUI 清理动画 tween 播放、TimelineUI 行动块 hover 状态通知、TimelineUI 入场动画编排状态、TimelineManager 敌方意图候选 collector、TimelineManager 敌方意图优先级 selector、TimelineManager 敌方意图目标地块 resolver、TimelineUI 行动容器几何 presenter、TimelineUI 行动方块视觉 presenter、TimelineUI 整体形状视觉 presenter、TimelineUI 视觉配置资源、TimelineUI visual config reader、CustomCard description parser、CustomCard node bridge、CustomCard 时间轴形状解析、CustomCard 效果范围解析、CustomCard 选中跟随视觉 presenter、CustomCard hover shader presenter、CustomCard tooltip bridge、CustomCard map conditional bridge、Timecoin GlobalTimecoin bridge、Timecoin hourglass shader controller、Timecoin shake tween builder、Timecoin feedback animation runner、EnemyIntent tooltip text builder、EnemyIntent status keyword tooltip presenter、EnemyIntent tooltip position helper、Tile timeline shape parser、Tile intent action factory、Tile intent action data builder、Tile health state rules、Tile death execution controller、Tile damage protection rules、Tile texture state selector、OutScene 房间结算 payload 控制器、OutScene 章节揭示动画 runner、OutScene payload bridge、OutScene camera limit controller。
```
