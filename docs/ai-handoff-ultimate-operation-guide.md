...
当前模块化工作已经完成多条主线。当前已拆脚本模块共 `173` 个，另有 `4` 个默认 Resource 文件；`docs/modularized-files-ultimate-operation-guide.md` 对这些脚本和资源路径的覆盖缺失数为 `0`。
...
| `scene/in_scene/timeline/TimelineManager.gd` | 约 493 行，`timeline/manager_modules/` 下已有 1 个规则模块。敌方意图优先级读取、优先级分层排序、同级候选过滤和同级可放置选择已拆到 `TimelineEnemyIntentPrioritySelector.gd`。 | 后续若继续，只单独评估敌方意图候选收集或目标地块映射；不要同批修改 `grid` 占用、`place_action()`、`TimelineAction.action_data` 契约和 UI 表现。 |
...
| 9 | `scene/in_scene/timeline/TimelineManager.gd` | 敌方意图优先级和同级可放置选择已拆出；候选收集、目标地块映射、action priority 写入、重判失效和中途移除仍在主脚本。 | 后续只单独评估候选收集或目标映射，不要同批改 `grid`、`place_action()`、结算或 UI 表现。 |
...
当前已拆脚本模块共 173 个，另有 4 个默认 Resource 文件，docs/modularized-files-ultimate-operation-guide.md 覆盖缺失为 0。不要继续机械拆 hex_map.gd、in_scene.gd、AcquireReward.gd、RemoveReward.gd 或 CraftReward.gd 的确认关闭链。下一阶段优先处理：
