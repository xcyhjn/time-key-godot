...
> 当前迁移已通过评估并完成 Wave 02B2A。下一阶段应完整读取并执行同目录的 `NEXT_STAGE_DECOUPLING_PROMPT.md`；解耦完成后再执行 `NEXT_STAGE_REMAINING_CARDS_PROMPT.md`。本文、`NEXT_STAGE_COMBAT_PROMPT.md` 与 `NEXT_STAGE_EFFECTS_PROMPT.md` 保留为总规范和已完成阶段依据，不应重复执行已经关闭的 Wave 00/01/02A/02B1/02B2A。
