...
- `animation/TimecoinTweenStateController.gd`：维护 active_tweens 列表的登记、容量清理、失效引用过滤、全部停止和完成后移除；不创建 Tween，不拼接动画片段，也不重置 UI 位置、缩放或颜色。

## 不要继续硬拆

- 不要重复拆 `GlobalTimecoin` 查找或沙漏 shader 控制。
- 不要重复拆获得、消耗和不足反馈动画 runner。
- 不要重复拆 `active_tweens` 状态 controller。
- 不要同批改数值来源、信号协议和动画收尾状态。
- 不要把 `active_tweens` 管理直接塞进 shader controller、shake builder 或 feedback runner。

## 后续可做

TimecoinUI 的低风险拆分面已经基本收口。下一步优先转向其他文件；如果继续 TimecoinUI，先重新审查剩余函数，不要为了降行数硬拆 composition root。若只是调整获得、消耗和不足动画的颜色、缩放、闪烁或时长，优先改 `TimecoinFeedbackAnimationRunner.gd`，不要移动触发时机。
