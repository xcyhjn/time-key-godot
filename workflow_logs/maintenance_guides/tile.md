...
`scene/in_scene/tile.gd` 是地貌/建筑实体基类，承担地貌基础属性、状态组件、结算奖励、敌人意图协议、视觉挂接、血量状态和贴图切换。时间轴 shape 矩阵解析、默认敌人意图 `TimelineAction` 构造、血量纯规则计算、Broken 后死亡收尾执行、protected 受击吸收规则和贴图归属切换判断已经拆出，但旧入口仍由主脚本负责写回成员、发信号或执行副作用。
...
- 不要重复拆死亡执行 controller、protected 受击规则或贴图归属选择规则，也不要同批拆子类 picker 策略和子类敌人意图行动数据统一。
- 不要把 `owner_battle`、`map_data`、运行时地貌节点注册成 Resource。
- 不要在 tile 模块里直接查找场景树；需要地图信息时由主脚本或调用方传入。

## 后续可做

下一步可单独评估多个敌方地貌子类的意图 action 数据是否能统一，或停止 Tile 转向其他大文件。若继续 Tile，必须保持 `Blood_change`、Broken 贴图、`tile_topology_changed`、`effect_range`、`invalid_reason` 和 `TimelineAction` 数据契约不变。
